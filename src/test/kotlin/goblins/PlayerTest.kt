...
//        countGameTurns(testGame1) shouldEqual 27730
//    }
//
//    @Test
//    fun `Should run game on real input`(){
//        countGameTurns(realGame1).let { print(it) }
//    }

    @Test
    fun `Should find game result where no elves die in test game`(){
        noElvesDie(testGame1) shouldEqual 4988
    }

    @Test
    fun `Should find game result where no elves die in real game`(){
        noElvesDie(realGame1).let { println(it) }
    }
}